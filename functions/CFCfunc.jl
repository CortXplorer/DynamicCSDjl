function visSpectrum(layCSDst, sr = 1000)

    t   = [1:Int(1000/sr):length(layCSDst)...] #time accounting for sr
    dt  = t[2] - t[1]   # sampling interval
    fNQ = 1 / dt / 2    # Nyquest is half sampling rate (fraction: 0.5)
    N   = length(t)     # number of data points 
    T   = length(layCSDst) # the total duration of data (T = N here)

    x  = hanning(N) .* layCSDst[:] # multiply data by hanning taper
...
function thetagamma_filter(layCSDst, sr, NQ, signal, takepic=1)
    ...
    Vlo = signal.filtfilt(b, 1, layCSDst) #PyCall
