...
* Trials were averaged by bins of 5. There are ~50 trials for each Animal, resulting in 10 bins.

# Model

`[Metric] ~ Group * Measurement + (1|Animal/TrialBin)`

Baseline (intercept): `Group` = "KIT" & `Measurement` = "preLaser"
...
all_data <- raw_data %>% 
  ...
  mutate(
    Measurement = case_when(
      ...
      TRUE ~ "postLaser"),
    Measurement = factor(Measurement, levels = c("preLaser", "postLaser")),
    Animal = factor(Animal),
    Group = factor(Group, levels = c("KIC", "KIT", "KIV")),
    TrialBin = case_when(
      TrialNumber %in% 1:5 ~ 1,
      TrialNumber %in% 6:10 ~ 2,
      TrialNumber %in% 11:15 ~ 3,
      TrialNumber %in% 16:20 ~ 4,
      TrialNumber %in% 21:25 ~ 5,
      TrialNumber %in% 26:30 ~ 6,
      TrialNumber %in% 31:35 ~ 7,
      TrialNumber %in% 36:40 ~ 8,
      TrialNumber %in% 41:45 ~ 9,
      TrialNumber %in% 46:50 ~ 10
    ),
    TrialBin = factor(TrialBin)
  ) %>% 
  group_by(Group, Animal, Layer, Measurement, ClickFreq, TrialBin) %>% 
  mutate(
...
for (freq in stimfreq){
  ...
  for(layer in layers){
    ...
    for(metric in metrics){
      ...
      #   show.aic = TRUE, title = "Summary of the LMM", file="LMM.html")
      # 
      # htmltools::includeHTML("LMM.html")
      ...
      df <- pairs(lsmeans(model_final, ~ Measurement | Group), adjust = "tukey") %>%
        data.frame()
      ...
      df %>%
        kable(escape = F, digits = 4, caption = "Multiple Comparisons (between Measurements)") %>%
        kable_classic_2(full_width = F) %>%
      ...
      df <- pairs(lsmeans(model_final, ~ Group | Measurement), adjust = "tukey") %>%
        data.frame()
...
# model_final <- lme(
#         fixed = as.formula(paste0(metric, "~ Group*Measurement")),
#         random = ~1|Animal/TrialBinAnimal,
#         data = model_data)
# 
# summary(model_final)
```
